 ...
 *
 * Es el mismo contenedor que usan las cards de Estadísticas, extraído acá
 * porque todas las secciones de Mi día repetían el header entero.
 *
 * Es más compacta que esas otras cards —`p-3.5`, chip de 28px, título en
 * 0.95rem— porque Mi día es la pantalla que se recorre entera varias veces por
 * día: cada escalón menos de aire adentro de una card se traduce en más filas
 * arriba del pliegue. Las pantallas que muestran una card sola —los gráficos de
 * Perfil, el panel de Tareas— se quedan en `p-5`: ahí el aire no cuesta nada.
 *
 ...
 * filas pierden su `px-4`: el aire lateral ya lo pone el `p-3.5`. Eso se apaga
...
export function SeccionCard({
  ...
  return (
    <section
      ...
      className={`mb-4 min-w-0 rounded-2xl border border-border bg-surface p-3.5 shadow-sm [&>ul>li]:px-0 [&>ul>li>a]:px-0 [&>ul>li>div]:px-0 ${className}`.trim()}
    >
      <header className="mb-1.5">
        <div className="flex flex-wrap items-center gap-2">
          <span
            aria-hidden
            className={`flex size-7 shrink-0 items-center justify-center rounded-[10px] ${TONOS[tono]}`}
          >
            {icono}
          </span>
          ...
          {badge && (
            <span
              className={`rounded-full px-2.5 py-0.5 text-[0.7rem] font-bold ${TONOS[tono]}`}
            >
              {badge}
            </span>
          )}
          ...
          {verTodos && (
            <Link
              to={verTodos.ruta}
              className="ml-auto text-[0.78rem] font-semibold whitespace-nowrap text-primary hover:underline"
            >
              {verTodos.texto}
            </Link>
          )}
        ...
        <p className="mt-0.5 text-[0.78rem] text-ink-3">{subtitulo}</p>
      </header>
