...
export function IndicadorCupo({ cupo }: { cupo: CupoEquipo }) {
  // Sin límite legible no se puede dibujar una barra ni avisar de nada: se
  // muestra el conteo a secas. El tope real lo aplica el server al invitar.
  if (cupo.limite == null) {
    return (
      <p className="text-[0.8rem] text-ink-2">
        <b className="text-ink tabular-nums">{cupo.usados}</b>{' '}
        {cupo.usados === 1 ? 'persona en el equipo' : 'personas en el equipo'}
      </p>
    )
  }
